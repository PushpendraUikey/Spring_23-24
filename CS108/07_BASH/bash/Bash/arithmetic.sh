...
variance=0
while read line; do
	variance=$(( ( line - average)**2 + variance))
done < "$file_path"

variance=$((variance/i)) | bc -l

# Display the result
echo "The average of all the no present is: $average"
echo "The variance of all the no present is: $variance"
